...
bool thirteenth = false; // 10 sec newyork popular icons
bool fourteenth = false; // 15 sec telling all cities are neighborhoods.
...
//seventh scene
ofVec2f seattlePos1, seattlePos2, seattlePos3, seattlePos4, seattlePos5;

ofVec2f seattlePos[5] = {seattlePos1, seattlePos2, seattlePos3, seattlePos4, seattlePos5};
...
// thirteenth scene - positions of images
ofVec2f momaPos, metrocardPos, ilovenyPos, yellowCabPos, libertyPos;
ofVec2f nyImages[5] = {momaPos, metrocardPos, ilovenyPos, yellowCabPos, libertyPos};


// images position - for moving
ofVec2f sojuPos, ramenPos, samsungPos, lgPos, kimchiPos;
...
// seventh scene
    starbucks.load("starbucks.png");
    microsoft.load("microsoft.png");
    costco.load("costco.png");
    spaceneedle.load("spaceNeedle.png");
    boeing.load("boeing.png");
    
    for(int i=0; i<5; i++ ) {
        seattlePos[i].set((170*i),ofRandom(100, 450));
    }
...
// thirteenth scene - icons of new york
    moma.load("moma.png");
    metrocard.load("metrocard.png");
    iloveny.load("iloveny.png");
    yellowCab.load("yellowCab.png");
    liberty.load("liberty.png");
    
// fourteenth scene
    earth.load("earth.png");
    airplaneLast.load("airplane-last.png");
    
    for (int i=0; i<5; i++) {
        nyImages[i].set(170*i,ofRandom(100,450));
    }
...
void ofApp::update() {
    // time update
    sec = (ofGetFrameNum() / 60) % 135;

    // scene times
    if(story == true) {
        if(sec >= 0 && sec <= 10) { // map specification
            first = true;
            ...
            nineth = false;
            tenth = true;
        } else if (sec >= 100 && sec < 105) {
            tenth = false;
            eleventh = true;
        } else if ( sec >= 105 && sec < 115) {
            eleventh = false;
            twelvth = true;
        } else if ( sec >= 115 && sec < 125) {
            twelvth = false;
            thirteenth = true;
        } else if ( sec >= 125 && sec < 140) {
            thirteenth = false;
            fourteenth = true;
            story = false;
        }
...
// eighth scene - mountainRainier
    ...
    rainLength = ofMap (mouseX, 0, width, 50, 0);
...
// eleventh scene - plane scene
    ...
    hour2 = ofMap(sec, 100, 105, 1, 7);
    
// thirteenth scene - moving cab
    nyImages[3].x += 1;
    if( nyImages[3].x >= ofGetWidth()) {
        nyImages[3].x = 0;
    }
    
    nyImages[4].y -= 1 * statueD;
    if( nyImages[4].y <= 100 || nyImages[4].y >= 450) {
        statueD *= -1;
    }
...
//    cout << figure1.x << endl;
    ...
    if(story == false) {
        ofSetColor(black);
        bebasBig.drawString("Thank You", ofGetWidth()/2-150, ofGetHeight()/2);
    }
// ------ events
    if ( first ) {
        ofBackground(255);
        ...
        if(memory == true) {
            ofSetColor(black);
            bebasSmall.drawString("In  my  memory...", ofGetWidth()/2-100, ofGetHeight()/2);
        } else if (different == true) {
            ofSetColor(black);
            bebasSmall.drawString("Each  place  was  different  in  many  aspects..",ofGetWidth()/2-300, ofGetHeight()/2);
        }
//        ofDisableAlphaBlending();
    ...
    bebasBig.drawString(ofToString(fourthTimer),width-100,100);
        ...
        if(flagStatus == 1 ){
         ...
         bebasSmall.drawString("This  is  Han - Geul", ofGetWidth()/2-150, 100);
        ofBackground(255);
        ofSetColor(white);
        ...
        ofSetColor(black);
        bebasSmall.drawString("What's popular in Seattle?", ofGetWidth()/2 - 170, 150);
        ofSetColor(white);
        starbucks.draw(seattlePos[0]);
        microsoft.draw(seattlePos[1]);
        costco.draw(seattlePos[2]);
        spaceneedle.draw(seattlePos[3]);
        boeing.draw(seattlePos[4]);
        ...
        bebasBig.drawString("Seattle" , 100, 300);
        bebasBig.drawString("New York" , ofGetWidth()-300, 300);
    ...
    ofDrawRectangle(figure3.x-105,figure3.y-50,210,50);ofDrawRectangle(figure3.x-85,figure3.y-150,170,100);ofDrawRectangle(figure3.x-65,figure3.y-370,130,220);ofDrawRectangle(figure3.x-45,figure3.y-420,90,50);ofDrawRectangle(figure3.x-35,figure3.y-450,70,30);ofDrawRectangle(figure3.x-25,figure3.y-470,50,20);
        ...
        ofSetColor(black);
        bebasSmall.drawString("Iconic things in New York?", ofGetWidth()/2 - 170, 150);
        
        ofSetColor(white);
        moma.draw(nyImages[0],300,300);
        metrocard.draw(nyImages[1],300,300);
        iloveny.draw(nyImages[2],300,300);
        yellowCab.draw(nyImages[3],300,300);
        liberty.draw(nyImages[4],300,300);
        
    }
        else if (fourteenth) {
        ofBackground(blueBlack);
        ofSetColor(skyBlue);
            ofDrawCircle(ofGetWidth()/2,ofGetHeight()/2, 250); // radius = 250;
            earth.draw(0,0);
            
            bebasSmall.drawString("Where ever we are, we are all neighbors on the earth", 150, 100);
...
// twelveth scene -- switch being clicked.
    if(sec > 105 && sec < 115) {
        if(clicked == true) {
            cout << "Day Time" << endl;
            clicked = false;
